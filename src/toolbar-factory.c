                         ...
                         toolbar_info * info)
{
    ...
    for (i = 0; i < G_N_ELEMENTS(tm_toolbar_options); i++) {
        ...
        g_menu_item_set_action_and_target(item, "toolbar.set-style",
                                          "i", option->style);
    ...
    for (i = 0; i < G_N_ELEMENTS(tm_toolbar_options); i++) {
        ...
        if (option->style == default_style) {
            ...
            g_menu_item_set_action_and_target(item, "toolbar.set-style", "i", -1);
    ...
    if (gtk_widget_is_sensitive(toolbar)) {
        ...
        g_menu_append(section, _("_Customize Toolbars…"), "toolbar.customize");

        g_menu_append_section(menu, NULL, G_MENU_MODEL(section));
        g_object_unref(section);
    }

    popup_menu = gtk_popover_new_from_model(toolbar, G_MENU_MODEL(menu));
    g_object_unref(menu);
